...
public class WebFormContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Person>(person =>
        {
            ...
            person.Property(p => p.IdentificationType).IsRequired().HasMaxLength(20);
            person.Property(p => p.CreationDate).IsRequired().HasDefaultValueSql("GETDATE()");
            person.Property(p => p.IdentificationNumberConcat).HasComputedColumnSql("[IdentificationNumber] + '-' + [IdentificationType]")
                .ValueGeneratedOnAddOrUpdate();
            person.Property(p => p.FullNameConcat).HasComputedColumnSql("[FirstName] + ' ' + [LastName]")
                .ValueGeneratedOnAddOrUpdate();
        });
